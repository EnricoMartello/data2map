...
img_list_column = [
  [ sg.Text("Seleziona il file contenente la piantina:\n") ],
	[ sg.Listbox( values=[], enable_events=True, size=(40, 20), key="-IMG LIST-" ) ]
]

xcl_list_column = [
	[ sg.Text("Seleziona il file contenente le misure:\n") ],
	[ sg.Listbox( values=[], enable_events=True, size=(40, 20), key="-DATA LIST-" ) ]
]
...
while True:
	...
	if event == "-WORKING FOLDER-":
		...
		try:
			imgname = os.path.join( values["-WORKING FOLDER-"], values["-IMG LIST-"][0] )
		except:
			pass

	elif event == "-DATA LIST-":  # A file was chosen from the listbox
		try:
			dataname = os.path.join( values["-WORKING FOLDER-"], values["-DATA LIST-"][0] )
		except:
			pass
